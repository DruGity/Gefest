    ...
    <li class="">
        ...
        function getDbins($settings, $item = false, $type = 'articles', $action = 'add', $use_translations = -1)
        {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                elseif ($name == 'products_id') {                                                                                           // products_id
                    $product_id = '';
                    //$main_product_id = 0;
                    $prod_ids = $_POST['products_id'];
                    $ccount = count($prod_ids);
                    for ($ci = 0; $ci < $ccount; $ci++) {
                        //if ($ci == 0) $main_product_id = $prod_ids[$ci];              // товары
                        $product_id .= '*' . $prod_ids[$ci] . '*';
                        if (($ci + 1) < $ccount)
                            $product_id .= '|';
                    }
                    //vdd($main_category_id);
                    $dbins['products_id'] = $product_id;
                    //$dbins['main_product_id'] = $main_product_id;

                    // $parent_product_id = 0;
                    // $parent_product = $CI->model_categories->getParentById($main_category_id);
                    // // vdd($parent_category);
                    // if (isset($parent_category['id']))
                    //     $parent_category_id = $parent_category['id'];
...
//vd($file);
    ...
    <li id="image-<?= $image['id'] ?>" class="article-image ui-state-default">
        <div style="float: left">
            ...
            <div>
                <label for="image_num_<?=$image['id']?>">Позимция:</label>
                <input id="image_num_<?=$image['id']?>" class="image_num form-control" type="number" image_id="<?= $image['id'] ?>"
                       name="num" value="<?=$image['num']?>" /><br/>
            </div>
        ...
        <div style="float: left">
            ...
            if ($articles_adding_images_types) {
                ...
                if (is_array($typesArr)) {
                    ?>
                    Тип:<br/>
                    <select  class="form-control adding-image-type"
