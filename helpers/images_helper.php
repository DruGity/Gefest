...
include X_PATH . '/application/third_party/ResizeImage.php';
include X_PATH . '/application/third_party/ImageResize.php';
...
function resizeImage($image_path, $width, $height, $directory = false, $quality = 80)// ресайзит по отношению сторон
{
    if (isOurDomain($image_path)) {
        $image = $_SERVER['DOCUMENT_ROOT'] . $image_path;
        ...
        if ($w == $width && $h == $height) return $image_path;
            ...
            if ($directory == false) {
                if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $dir . "/" . $width . "x" . $height))
                    mkdir($_SERVER['DOCUMENT_ROOT'] . $dir . "/" . $width . "x" . $height, 0777, true);
                $resize->saveImage($_SERVER['DOCUMENT_ROOT'] . $dir . "/" . $width . "x" . $height . "/" . $c, $quality);  // r - resised

                return $dir . $width . "x" . $height . "/" . $c;
            } else {
                if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $directory))
                    mkdir($_SERVER['DOCUMENT_ROOT'] . $directory, 0777, true);
                $resize->saveImage($_SERVER['DOCUMENT_ROOT'] . $directory . "/" . $c,$quality);
                return $directory . "/" . $c;
            }
        }
...
// Перебираем наш массив слов
    $arrCount = count($arr);
    for ($i = 0; $i < $arrCount; $i++) //foreach($arr as $word)
    {
        $word = $arr[$i];
...
function is_image($filename)
{
    ...
    if (!$img) return false;
    elseif (!array_key_exists($img[2], array('1' => 'gif', '2' => 'jpg', '3' => 'png', '4' => 'ico'))) return false;
    else return true;
}
...
function uploadFile($config = array('name' => false, 'file' => 'userfile', 'type' => 'articles', 'encrypt_name' => true))
{
    ...
    $fileExtantion = strtolower($fileExtantion);
    ...
    $config['allowed_types'] = 'jpg|png|gif|jpe|pdf|doc|docx|xls|xlsx|ico';
    $config['max_size'] = '0';
    $config['max_width'] = '0';
    $config['max_height'] = '0';
