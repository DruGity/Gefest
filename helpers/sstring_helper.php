...
function cropText($text, $charsCount = 160, $toLastWord = true, $lastSymbol = ' ', $stripTags = false, $tagsAllowed = ''){
    if($stripTags)
        $text = strip_tags($text, $tagsAllowed);

    $substr = mb_substr($text, 0, $charsCount);
    if($toLastWord){
        $pos = mb_stripos($substr, $lastSymbol);
        if($pos){
