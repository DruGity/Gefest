        ...
        if ($elements) {
            foreach ($elements as $elem) {
                ...
                <tr id="tr-<?= $elem['id'] ?>" class="list<?php if ($elem['active'] == 0) echo ' no-active'; ?>">
                    ...
                    <td><strong>Раздел</strong>: <?= $elem['template'] ?><br/><strong>Контент:</strong> <?=$elem['content_template']?></td>
...
class MenuTree
{
    ...
    function __construct($position = 'top', $config = false)
    {
            ...
            else $this->config['itemOpenTag'] = '<li class="dropdown">';
...
{
    private $config;
    private $linkTemplate = "<a class='menu-itm' href = \"http://%s\">%s</a>";
    private $imgTemplate = "<img src = \"%s\" alt=\"%s\" title=\"%s\" />";
...
{
    private $config;
    private $linkTemplate = "<a class='mob-menu-itm' href = \"http://%s\">%s</a>";
    private $imgTemplate = "<img src = \"%s\" alt=\"%s\" title=\"%s\" />";
