        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                ...
                elseif ($fieldtype['view'] == 'color') {                                                                    // COLOR
                    ...
                    if (isset($article[$name])) {
                        echo '<input type="text" class="form-control" name="' . $name . '" value="' . $article[$name] . '" />';
                    }
                    echo '
               ...
               // var_dump($article['image']);
                if (isset($article[$name]) && $article[$name] != '') {
                    ...
                    echo '<input id="inp_' . $name . '" type="text" class="form-control" name="' . $name . '" value="' . $article[$name] . '" />';
                }
                ...
                elseif ($fieldtype['view'] == 'multiple-select') {                                                             // Товары
                    if ($name == 'products_id') {     // Выводим список товаров
                        $pref = "";
                        if (isset($typeOfContent)) $pref = $typeOfContent . '_';
                        //$multiple_categories = getOption($pref . 'multiple_categories');

                        $prod_ids = false;
                        if (isset($article['products_id'])) {
                            $prod_ids = explode('|', $article['products_id']);
                            if (is_array($prod_ids)) {
                                $count = count($prod_ids);
                                for ($i = 0; $i < $count; $i++) {
                                    //echo (int) $cat_ids[$i].'<hr/>';
                                    $prod_ids[$i] = str_replace('*', '', $prod_ids[$i]);
                                }
                            }
                        }
                        //vdd($products);
                        ...
                        if ($products) {
                            $optionsTree = new AdminElementsTree('category');
                            $optionsTree->setSelectedOptions($prod_ids);
                            echo $optionsTree->createOptionsTreeForSelect($products, 0);
                        }
                        echo '</SELECT>';
                        if ($multiple_categories == 1) {
                            echo '<script>
