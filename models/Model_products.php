...
class Model_products extends CI_Model
{
    ...
    function getLastProducts($per_page = -1, $from = -1, $active = -1, $category_id = -1, $order_by = "DESC", $sort_by = 'id')
    {
...
//            $this->db->where('created_by', $created_by);
    ...
    function getRelatedProducts($product_id = -1, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC", $sort_by = 'num', $rayon = -1, $not_article_id = false)
    {
        if ($active != -1) {
            $this->db->where('active', $active);
        }
