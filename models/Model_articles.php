...
class Model_articles extends CI_Model
{
    ...
    function getRandomArticles($per_page = -1, $from = -1, $order_by = "DESC", $active = -1, $not_in_categories = false)
    {
        if ($active != -1)
            $this->db->where('active', $active);
        if ($not_in_categories) {
    ...
    function getArticlesByParam($param, $value, $per_page = -1, $from = -1, $active = -1,$category_id = -1, $order_by = "DESC", $not_in_categories = false)
    {
    ...
    function getLastArticleAkciya( $per_page = -1, $from = -1, $active = -1,$category_id = -1, $order_by = "DESC")
    {
        $this->db->where('akciya', 1);
        if ($active != -1){
            $this->db->where('active', $active);
        }
        if ($category_id != -1){
    ...
    function getArticlesByCount($per_page = -1, $from = -1, $active = -1, $not_in_categories = false)
    {
        if ($active != -1)
            $this->db->where('active', $active);
        if ($not_in_categories) {
                $this->db->like('category_id', '*' . $not_in_categories . '*');
        }
        //$this->db->order_by('id', $order_by);
        if ($per_page != -1 && $from != -1)
            $this->db->limit($per_page, $from);
    ...
    function getArticlesByCategory($category_id = -1, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC", $sort_by = 'num', $rayon = -1, $not_article_id = false)
    {
        if ($active != -1) {
            $this->db->where('active', $active);
        }
        ...
        if($sort_by == 'voites') {
            //$orderByStr = "voites, ASC, lastname ASC, name, ASC";
            //$this->db->order_by($orderByStr);
            $this->db->order_by($sort_by, $order_by);
        }else $this->db->order_by($sort_by, $order_by);
    ...
    {
        if ($onlyFromCategoryId != false)
            $this->db->like('category_id', "*".$onlyFromCategoryId."*");
    ...
    function getLastArticlesForPagination($from=0, $per_page=20, $onlyFromCategoryId = false)
    {
        if ($onlyFromCategoryId != false)
            $this->db->like('category_id', "*".$onlyFromCategoryId."*");
        $this->db->order_by('num', 'DESC');
...
//			$this->db->where('active', $active);
    ...
    public function getLastAkcii()
    {
        $ret = $this->db->query("SELECT * FROM articles WHERE akciya=1")->result_array();
        return $ret;
    }
    ...
    public function getByParseUrl($parse_url){
        $this->db->where('parse_url', $parse_url);
        $this->db->limit(1);
        $ret = $this->db->get('articles')->result_array();
        if (isset($ret[0])) return $ret[0];
    ...
    function getArticlesByFirstLetter($letter)
    {
        $this->db->like('name', $letter, 'after'); 
        return $this->db->get('articles')->result_array();
    }

    function getArticleByNum($num, $active = -1,$category_id = -1)
    {
        if ($active != -1)
            $this->db->where('active', $active);
        if ($category_id != -1) {
    ...
    function getArticlesByCategoryAndFirstLetter($category_id = -1, $per_page = -1, $from = -1, $active = -1, $letter = -1, $order_by = "DESC", $sort_by = 'num', $not_article_id = false)
    {
        if ($active != -1) {
            $this->db->where('active', $active);
        }
    ...
    function getArticlesByCategoryAndFirstLetterP($category_id = -1, $per_page = -1, $from = -1, $active = -1, $letter = -1, $order_by = "DESC", $sort_by = 'num', $not_article_id = false)
    {
        if ($active != -1) {
            $this->db->where('active', $active);
        }
    ...
    function getArticlesBy($by, $byValue, $like = false, $category_id = -1, $per_page = -1, $from = -1, $sort_by = "date_unix", $order_by = "DESC",$active = 1, $not_in_categories = false){
        if($like)
            $this->db->like($by, $byValue);
        else
            $this->db->where($by, $byValue);
        if ($active != -1)
            $this->db->where('active', $active);
        if ($not_in_categories) {
            if (is_array($not_in_categories)) {
