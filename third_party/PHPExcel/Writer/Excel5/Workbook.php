...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
	 ...
	 * @param mixed		$parser			The formula parsers created for the Workbook
	 */
								...
								$parser )
	{
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			...
			$this->_parser->setExtSheet($phpSheet->getTitle(), $i);  // Register worksheet name with parsers

			$supbook_index = 0x00;
			$ref = pack('vvv', $supbook_index, $i, $i);
			$this->_parser->_references[] = $ref;  // Register reference with parsers
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			// (exclusive) either repeatColumns or repeatRows
				...
				// construct formula data manually because parsers does not recognize absolute 3d cell references
