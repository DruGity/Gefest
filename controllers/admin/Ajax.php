...
class Ajax extends CI_Controller
{
    ...
    public function cache($action)
    {
        ...
        if ($action == 'clear_files' || $action == 'clear_all') {
            $cachePath = X_PATH.'/application/cache/';
    ...
    public function categories($action)
    {
        if ($action == 'get_categories_templates') {                                                          // ПОЛУЧАЕМ СПИСОК ШАБЛОНОВ КАТЕГОРИЙ
            ...
            if ($type) {
                $this->load->helper('file');
                $files = get_filenames(X_PATH . '/application/views/' . SITE . '/categories/' . $type . '/');
                if(!$files)
            ...
            if ($type) {
                $this->load->helper('file');
                $files = get_filenames(X_PATH . '/application/views/' . SITE . '/' . $type . '/');
                if(!$files)
