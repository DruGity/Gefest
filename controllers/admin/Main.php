...
class Main extends CI_Controller
{
    ...
    public function tp(){
        if(isset($_POST['send'])){
            $msg = '
            Обращение с сайта: '.$_SERVER['SERVER_NAME'].'<br/>
            От: '.$_POST['name'].'<br/>
            Tel: '.$_POST['tel'].'<br/>
            E-mail: '.$_POST['email'].'<br/>
            Причина: '.$_POST['subject'].'<br/>
            Дата: '.date("Y-m-d H:i").'<br/>
            Текст обращения:<br> '.$_POST['message'].'<br/>
            
            ';

            loadHelper('mail');
            mail_send('support@fabrika-umh.com','Обращение из админки', $msg);
            $data['msg'] = 'Ваше обращение отправлено главному разработчику! Ожидайте ответа...';
        }
        $data['title'] = "Обращение в ТП панели администрирования";
        //$data['main'] = $this->ma->getMain();
        $data['main'] = false;
        $data['user'] = $this->model_users->getUserByLogin(userdata('login'));
        $data['head'] = $this->load->view('common/head.php',$data, true);
