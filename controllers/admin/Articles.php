...
class Articles extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            $data['articles'] = $this->articles->searchByName($_POST['search'],-1,$per_page,$page1);

            $data['articles1'] = $articles1 =  $this->articles->searchByName($_POST['search'],-1,10000,0);
            $count = count($articles1);
            ...
            $data['articles'] = $this->articles->getArticles($per_page,$page1);
            $data['articles1'] = $articles1 =  $this->articles->getArticles(10000,0);
            $count = count($articles1);
            $data['count1'] = ceil($count/$per_page);
            //Pagination
    ...
    public function category($id)
    {
        ...
        $data['title'] = "Статьи по категории";
        ...
        if (isset($_POST['search'])) {
            ...
            //////////
                $data['articles'] = $this->articles->getArticlesByCategory($id,$per_page, 0);
        }
...
// ФУНКЦИЯ НАПОЛНЕНИЯ МАССИВА ЗНАЧЕНИЯМИ ДЛЯ ДОБАВЛЕНИЯ ИЛИ РЕЛАКТИРОВАНИЯ СТАТЬИ
    ...
    public function add()
    {
        ...
        if (isset($_POST['add_article']) || isset($_POST['add_article_and_close'])) {
            ...
            if($article['image'] == '' && getOption('og_on') == 1){
                $this->load->helper('images_helper');
                if(getOption('og_on') == 1)
    ...
    public function edit($id) {
        ...
        // Google Maps //
        if(getOption('google_maps_in_articles') == 1) {
            $this->load->library('googlemaps');
            $onclick = '
        $("#coordsLat").val(event.latLng.lat());
        $("#coordsLng").val(event.latLng.lng());
...
//                        $this->excel->setExcelArray($arr, './upload/temp/import.xlsx');
    ...
    public function import_organisations()
    {
        ...
        if (isset($_FILES['userfile'])){
                 ...
                 if ($_FILES['userfile']['name'] != '') {
                     ...
                     $tmpfname = $_SERVER['DOCUMENT_ROOT'].'/upload/csv/'.$_FILES['userfile']['name'];
                 ...
                 for ($row = 2; $row <= $lastRow; $row++) {
                     ...
                     echo "</td><td>";
                      $content = $worksheet->getCell('B'.$row)->getValue();
                     echo "</td><td>";
                       $category_id = $worksheet->getCell('C'.$row)->getValue();
                     echo "</td><td>";
                       $complaint = $worksheet->getCell('D'.$row)->getValue();
                     echo "</td><td>";
                       $author = $worksheet->getCell('E'.$row)->getValue();
                     echo "</td><td>";
                     ...
                     echo "</td><td>";
                      $iframe = $worksheet->getCell('H'.$row)->getValue();
                     echo "</td><td>";
                      $district = $worksheet->getCell('I'.$row)->getValue();
                     echo "</td><td>";
                      $tags = $worksheet->getCell('J'.$row)->getValue();
                      echo "</td><td>";
                      $short_content = $worksheet->getCell('K'.$row)->getValue();
                      echo "</td><td>";
                      $image = $worksheet->getCell('L'.$row)->getValue();
                      echo "</td><td>";
                      $trash = $worksheet->getCell('M'.$row)->getValue();
                     ...
                     //парсинг телефона
                    list($resT, $trashT) = explode("telephone", $trash);
                    //парсинг телефона

                    //парсинг адреса
                    list($resA1, $trashA1) = explode("address", $trash);
                    list($resA2, $trashA2) = explode("latitude", $trashA1);
                    list($resEA1, $trashEA1) = explode(':"', $resA2);
                    list($adress, $trashEA2) = explode('";', $trashEA1);
                    //парсинг адреса

                    //парсинг телефона
                    list($resET1, $trashET1) = explode(':"', $trashT);
                    list($phone, $trashET2) = explode('";', $trashET1);
                    //парсинг телефона

                    //парсинг широты
                    list($resLat1, $trashLat1) = explode("latitude", $trash);
                    list($resLat2, $trashLat2) = explode("longitude", $trashLat1);
                    list($resELat1, $trashELat1) = explode(':"', $resLat2);
                    list($latitude, $trashELat2) = explode('";', $trashELat1);
                    //парсинг широты

                    //парсинг долготы
                    list($resLng1, $trashLng1) = explode("longitude", $trash);
                    list($resLng2, $trashLng2) = explode("streetview", $trashLng1);
                    list($resELng1, $trashELng1) = explode(':"', $resLng2);
                    list($longtitude, $trashELng2) = explode('";', $trashELng1);
                    //парсинг долготы
                    ...
                    $data = array(
                     ...
                     $new++;
                     $msg = '<p class="msg">Импорт успешно завершён! Денис, ты умничка!<br />Добавлено позиций: '.$new.'<br /></p>';
                     $data['msg'] = $msg;
                 }
             ...
             $data['title']  = "Импорт организаций CSV";
             $data['head'] = $this->load->view('common/head.php',$data, true);
             $data['header'] = $this->load->view('common/header.php',$data, true);
             $data['footer'] = $this->load->view('common/footer.php',$data, true);
    ...
    public function do_upload()
    {
        ...
        $config['upload_path']          = $_SERVER['DOCUMENT_ROOT'].'upload/csv';
    ...
    public function draganddrop()
    {
        ...
        $data['left_sidebar'] = $this->load->view('common/left_sidebar.php',$data, true);
        $data['footer'] = $this->load->view('common/footer.php',$data, true);
        $this->load->view('articles/draganddrop.tpl.php', $data);
