...
class Products extends CI_Controller
{
    ...
    public function index($category_id = false)
    {
        ...
        if (isset($_POST['search'])) {
            $products = $data['products'] = $this->products->searchByName($_POST['search']);
            $products1 =  $this->products->searchByName($_POST['search'],-1, 10000, 0);
             $count = count($products1);
             $data['count_products'] = ceil($count/$per_page);
        } else {
            //Pagination
            
            $products = $this->products->getProducts($per_page,$page1);
            $data['products1'] = $products1 =  $this->products->getProducts(10000,0);
            $count = count($products1);
            $data['count_products'] = ceil($count/$per_page);
...
//        }
    ...
    public function add()
    {
        ...
        $data['settings'] = $settings;
        $data['categories'] = $this->categories->getCategories(1, 'products');
        $data['products'] = $this->products->getProducts(500,0);
