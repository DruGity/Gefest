...
//    }
    ...
    public function category($url)
    {
        ...
        $user = $data['user'] = $this->model_users->getUserByLogin(userdata('login'));
        ...
        if(isset($_POST['add_comment']))
                {
                    ...
                    {
                        $data['color'] = 'red';
                        $data['msg'] = 'Сначала необходимо авторизоваться!';
                    }
                    else
                    {
                        ...
                        $data['color'] = 'green';
                        $data['msg'] = 'Комментарий успешно добавлен!';
...
*/
            ...
            $data['articles'] = $this->art->getArticlesByCategory($category['id'],500,0,1);
            ...
            if(!file_exists(TEMPLATE_PATH.$template))
                $template = '/categories/' . $category['type'] . '/' . $category['template'];
            ...
            if ($page) {
                ...
                if ($page['active'] != '1')
                    ...
                    $template = '/templates/pages/page.tpl.php';
                    if ($page['template'] != '') {
                        $template = '/templates/pages/' . $page['template'];
                        if(!file_exists(TEMPLATE_PATH.$template))
    ...
    public function subcategory($url, $parent_url)
    {
        ...
        if ($parent_url == 'brands') {
            ...
            if (($category) && ($parent)) {
                ...
                if ($category['type'] == 'articles')
                    $articles = $this->art->getArticlesByCategory($category['id'], $articles_pagination, 0, 1, $category['order_by'], $category['sort_by']);
                elseif ($category['type'] == 'products')
                ...
                if ($articles) {
                    ...
                    for ($i = 0; $i < $count; $i++)
                        $articles[$i] = translateArticle($articles[$i]);
                }
                $data['articles'] = $articles;
                $metaArr['robots'] = $category['robots'];
                $metaArr = generateMeta($category);
                ...
                $data['msg'] = false;
                $data['color'] = false;
                $comments = $mComments->getCommentsToArticle($article['id']);
                if($category['type'] == 'products')
                {
                    
                     $comments = $mComments->getCommentsToProduct($article['id']);
                     if(isset($_POST['add_comment_to_product']))
                    {
                        if (!$user)
                        {
                            $data['color'] = 'red';
                ...
                elseif(isset($_POST['answer']))
                {
                    ...
                    $dbins = array(
                        ...
                        'user_id'          => $user['id'],
                        'login'          => $user['login'],
                        'comment'      => post('answer_input'),
                        'ip'         => getRealIp(),
                        'active'           => 1,
                        'date'          => date("Y-m-d H:i"),
                        'time'           => date('H:i'),
                        'date_unix' => time(),
                        'reply_to' => post('comment_id'),
                        'likes' => 0,
                        'dislikes' => 0
                    );
                    $data['color'] = 'green';
                ...
                elseif(isset($_POST['submit-edit']))
                {
                    ...
                    if ($user['id'] == $comment['user_id'])
                    {
                        if (isset($_POST['comment_edit']))
                        {
                            ...
                            $data['msg'] = 'Ваш комментарий успешно изменен!';
                ...
                $template = '/'.$category['type'].'/new.tpl.php';
                if ($category['template'] != '') {
                    $template = '/'.$category['type'].'/' . $category['content_template'];
                    if(!file_exists(TEMPLATE_PATH.$template))
...
//                    $this->load->view('templates/articles/new.tpl.php', $data);
    ...
    public function archive1($date = false)
    {
        // архив
        if(!$date) $date = date("Y-m-d");


        $data['articles'] = $this->art->Archive($date);
        $data['title'] = $this->lang->line('category_search') . $date . getOption('global_title_' . $this->current_lang);
        $data['keywords'] = $date . getOption('global_keywords_' . $this->current_lang);
        $data['description'] = $this->lang->line('category_search') . $date . getOption('global_description_' . $this->current_lang);
        $data['robots'] = "noindex, follow";
        $data['seo'] = "";
        $this->load->view('templates/archive.tpl.php', $data);

    }

    public function archive($date = false)
    {
        // архив
        ...
        $archive = array(
            'name' => 'Новости за '.getWordDate($date),
            'robots' => 'noindex, follow'
        );


        $data['meta'] = $metaArr = generateMeta($archive);
        $data['category'] = array_merge($archive, $metaArr);
        
        $data['inArchive'] = true;

        $articles = $this->art->getArticlesBy('date', $date, true);
        if($articles)
        $data['articles'] = $articles;
        //vd($articles);
        $data['title'] = $this->lang->line('category_search') . $date . getOption('global_title_' . $this->current_lang);
